...
void
Light::Update(const BoundingBox& boundingBox)
{
...
rndColors(int count)
{
  ...
  for (int i = 0; i < count; i++) {
    glm::vec3 c = glm::rgbColor(glm::vec3(currentHue, 1.0, 0.5));
...
Scene::initBoundsFromImg(std::shared_ptr<ImageXYZC> img)
{
  ...
  initBounds(BoundingBox(glm::vec3(0.0f), dim));
}
#endif

void
Scene::initBounds(const BoundingBox& bb)
{
